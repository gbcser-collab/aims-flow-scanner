...
class _InvoiceScannerScreenState extends State<InvoiceScannerScreen> with WidgetsBindingObserver {
  ...
  Widget _review()=>ListView(padding:const EdgeInsets.all(16),children:[
    ...
    DropdownButtonFormField<InvoiceCategory>(initialValue:_category,dropdownColor:const Color(0xFF171A1F),style:const TextStyle(color:Colors.white,fontWeight:FontWeight.w700),decoration:_deco('Kategória'),items:InvoiceCategory.values.map((c)=>DropdownMenuItem(value:c,child:Text(c.hu))).toList(),onChanged:(v)=>setState((){_category=v??InvoiceCategory.other;if(_category==InvoiceCategory.tollVignette){_createVignette=true;_validFrom??=DateTime.now();_validUntil??=DateTime.now().add(const Duration(days:1));}})),
    ...
    if(_category==InvoiceCategory.tollVignette)...[
      SwitchListTile(value:_createVignette,onChanged:(v)=>setState(()=>_createVignette=v),activeThumbColor:const Color(0xFFE6B85C),title:const Text('Matrica / útdíj automatikus rögzítése',style:TextStyle(color:Colors.white,fontWeight:FontWeight.w800))),
      if(_createVignette)...[
        DropdownButtonFormField<String>(initialValue:_countries.contains(_country.text.toUpperCase())?_country.text.toUpperCase():null,dropdownColor:const Color(0xFF171A1F),menuMaxHeight:360,decoration:_deco('Ország'),items:_countries.map((c)=>DropdownMenuItem(value:c,child:Text(c))).toList(),onChanged:(v)=>setState(()=>_country.text=v??'')),
        const SizedBox(height:10),DropdownButtonFormField<String>(initialValue:_vignetteType,dropdownColor:const Color(0xFF171A1F),decoration:_deco('Típus'),items:const [DropdownMenuItem(value:'electronic',child:Text('Elektronikus matrica / e-útdíj')),DropdownMenuItem(value:'point_of_sale',child:Text('Helyszínen vásárolt / bizonylatos'))],onChanged:(v)=>setState(()=>_vignetteType=v??'electronic')),
