...
if(!$vehicle) aims_json(['ok'=>true,'plate'=>$plate,'autopilot'=>['mode'=>'idle','actionCode'=>'wait_job','severity'=>'info']]);
...
$gpsAge=$minutesAgo($point['captured_at']??null);$stationaryMin=$minutesAgo($state['stationary_since']??null);
$base=['mode'=>'idle','actionCode'=>'wait_job','severity'=>'info','jobId'=>null,'reference'=>'','seen'=>false,'accepted'=>false,'totalStops'=>0,'completedStops'=>0,'nextStop'=>null,'gpsAgeMinutes'=>$gpsAge,'stationaryMinutes'=>$stationaryMin,'stopDwellMinutes'=>null,'distanceKm'=>null,'etaMinutes'=>null,'plannedAt'=>null,'timeBufferMinutes'=>null,'sequenceAnomaly'=>false,'documentsRequired'=>false,'gpsFresh'=>$gpsAge!==null&&$gpsAge<20,'updatedAt'=>$now->format(DateTimeInterface::ATOM)];
...
foreach($stops as $row){
    $done=!empty($row['completed_at']);if($done)$completed++;
...
$allDone=count($stops)>0&&$completed===count($stops);$seen=!empty($job['driver_seen_at']);$accepted=!empty($job['driver_accepted_at']);
...
if($dwell!==null&&$dwell>=60)$severity='high';elseif($dwell!==null&&$dwell>=30&&$severity==='ok')$severity='warn';
if($accepted&&($gpsAge===null||$gpsAge>=20))$severity='high';if($sequenceAnomaly)$severity='high';
...
if($next&&$point&&is_numeric($point['latitude']??null)&&is_numeric($point['longitude']??null)){
    ...
    $speedKmh=(float)($point['speed_mps']??0)*3.6;$cruise=($speedKmh>=35&&$speedKmh<=110)?max(55,min(90,$speedKmh*.4+68*.6)):68;$etaMin=(int)ceil($distanceKm/$cruise*60);
}
$order=[];if(!empty($job['order_payload_json'])){$x=json_decode((string)$job['order_payload_json'],true);if(is_array($x))$order=$x;}
$planned=null;if($next)$planned=(string)($next['stop_type']==='delivery'?($order['delivery_time']??''):($order['pickup_time']??''));
$buffer=null;if($planned&&$etaMin!==null){try{$pt=new DateTimeImmutable($planned);$buffer=(int)floor(($pt->getTimestamp()-$now->getTimestamp())/60)-$etaMin;if($buffer<0)$severity='high';elseif($buffer<15&&$severity==='ok')$severity='warn';}catch(Throwable){}}
$out=$base;$out['mode']=$mode;$out['actionCode']=$action;$out['severity']=$severity;$out['jobId']=(int)$job['id'];$out['reference']=(string)$job['reference'];$out['seen']=$seen;$out['accepted']=$accepted;$out['totalStops']=count($stops);$out['completedStops']=$completed;
...
$out['stopDwellMinutes']=$dwell;$out['distanceKm']=$distanceKm;$out['etaMinutes']=$etaMin;$out['plannedAt']=$planned?:null;$out['timeBufferMinutes']=$buffer;$out['sequenceAnomaly']=$sequenceAnomaly;$out['documentsRequired']=$allDone;
